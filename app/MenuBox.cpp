...
MenuBox::MenuBox(acc::FilterInterface &filter, acc::TransactionInterface &trans, Builder &builder,
                 AddTransactionPopup &addTranPopup, FilterWindow &filterWindow)
    : m_tranInterface(trans),
      m_addTranPopup(addTranPopup),
      m_filterWindow(filterWindow),
      m_log("MenuBox"),
      m_filterBox(filter,builder)
{
...
void MenuBox::onNewFilter() { m_filterWindow.show(); }
...
void MenuBox::connectClick(Builder &builder, const std::string &widgetName,
